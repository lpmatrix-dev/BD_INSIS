...
EXECUTE IMMEDIATE 'CREATE OR REPLACE VIEW INSIS_GEN_BLC_V10.BLC_TRANSACTIONS_PROF_ACC
("BT_TRANSACTION_ID", "BT_TRANSACTION_CLASS", "BT_TRANSACTION_TYPE", "BT_TRANSACTION_DATE", "BT_GL_DATE", "BT_CURRENCY", "BT_AMOUNT", "BT_ASSESSED_VALUE", 
"BT_RATE", "BT_RATE_DATE", "BT_RATE_TYPE", "BT_VARIANCE", "BT_FC_CURRENCY", "BT_FC_AMOUNT", "BT_FC_VARIANCE", "BT_ITEM_NAME", "BT_NOTES", "BT_DUE_DATE", 
"BT_GRACE", "BT_GRACE_EXTRA", "BT_LEGAL_ENTITY", "BT_ORG_ID", "BT_ACCOUNT_ID", "BT_ITEM_ID", "BT_DOC_ID", "BT_APPLICATION_ID", "BT_TAX_ID", "BT_PARENT_TRX_ID", 
"BT_BILLING_RUN_ID", "BT_STATUS", "BT_OPEN_BALANCE", "BT_PAID_STATUS", "BT_CHARGE_TO", "BT_REF_DOC_ID", "BT_CREATED_ON", "BT_CREATED_BY", "BT_UPDATED_ON", 
"BT_UPDATED_BY", "BT_ATTRIB_0", "BT_ATTRIB_1", "BT_ATTRIB_2", "BT_ATTRIB_3", "BT_ATTRIB_4", "BT_ATTRIB_5", "BT_ATTRIB_6", "BT_ATTRIB_7", "BT_ATTRIB_8", 
"BT_ATTRIB_9", "ACC_PARTY_ROLE_ID", "ACC_PROFILE", "ACC_PARTY", "ACC_BILL_TO_SITE", "ACC_REFERENCE", "ACC_NOTES", "ACC_LAST_DATE", "ACC_NEXT_DATE", 
"ACC_BILLING_SITE_ID", "ACC_COLLECTION_SITE_ID", "ACC_PAYMENT_SITE_ID", "ACC_BILL_METHOD_ID", "ACC_BILL_CYCLE_ID", "ACC_BILL_PERIOD", "ACC_BILL_HORIZON", 
"ACC_MIN_RETAIN_DAYS", "ACC_MAX_RETAIN_DAYS", "ACC_DUE_PERIOD", "ACC_GRACE_PERIOD", "ACC_MIN_BILL_AMOUNT", "ACC_MIN_REFUND_AMOUNT", "ACC_MIN_AMOUNT_ACTION_ID", 
"ACC_RECEIPT_METHOD_ID", "ACC_COLLECTION_SET_ID", "ACC_INTEREST_METHOD", "ACC_PAYMENT_METHOD_ID", "ACC_PAY_HORIZON", "ACC_NET_HORIZON", "ACC_PAY_GROUP_ID", 
"ACC_BASE_PROFILE_ID", "ACC_STATUS", "ACC_ATTRIB_0", "ACC_ATTRIB_1", "ACC_ATTRIB_2", "ACC_ATTRIB_3", "ACC_ATTRIB_4", "ACC_ATTRIB_5", "ACC_ATTRIB_6", "ACC_ATTRIB_7", 
"ACC_ATTRIB_8", "ACC_ATTRIB_9", "BI_LEGAL_ENTITY_ID", "BI_SOURCE", "BI_AGREEMENT", "BI_COMPONENT", "BI_DETAIL", "BI_ITEM_TYPE", "BI_PARTY", "BI_BILL_TO_SITE", 
"BI_INSURANCE_TYPE", "BI_AGENT", "BI_OFFICE", "BI_RETAINED", "BI_PRIORITY", "BI_BILL_CURRENCY", "BI_BILL_MODE", "BI_ITEM_NAME", "BI_STATUS", "BI_ATTRIB_0", 
"BI_ATTRIB_1", "BI_ATTRIB_2", "BI_ATTRIB_3", "BI_ATTRIB_4", "BI_ATTRIB_5", "BI_ATTRIB_6", "BI_ATTRIB_7", "BI_ATTRIB_8", "BI_ATTRIB_9", "DOC_TYPE", "DOC_NUMBER", 
"DOC_ISSUE_DATE", "DOC_AMOUNT", "TECH_BRANCH", "RI_FAC_FLAG", "DOC_START_DATE", "DOC_END_DATE", "REF_DOC_NUMBER", "POLICY_START_DATE", "POLICY_END_DATE", "POL_END_FLAG", 
"PAY_WAY", "OFFICE_GL_NO", "POL_END_PARTY_NAME", "POL_END_PARTY", "DOC_PREM_AMOUNT", "DOC_DUE_DATE", "PROTOCOL_NUMBER", "SALES_CHANNEL", "INTRMD_TYPE", "VAT_FLAG", 
"BUS_UNIT", "TECH_BR_RR", "TRX_SAVING_AMOUNT", "POLICY_CLASS", "ACC_PRIORITY", "POLICY_NO", "PENDING_FLAG") AS 
SELECT bt.transaction_id  bt_transaction_id,
    ...
    (SELECT to_date(bi.attrib_5,''YYYY-MM-DD'') + 1 -- LPVS-12 add + 1
     FROM blc_installments bi
     WHERE bi.transaction_id = bt.transaction_id
    ...
    (SELECT trunc(pp.insr_end) -- LPVS-12 remove - 1
     FROM blc_installments bi,
          policy pp
     WHERE bi.transaction_id = bt.transaction_id
     AND to_number(bi.policy) = pp.policy_id
    ...
    insis_blc_global_cust.cust_acc_util_pkg.Get_Prof_Priority_2(br.run_mode, br.bill_method_id, bi.attrib_7) acc_priority, --LAP85-97
    pp.policy_name    AS policy_no, -- LPV-819
    decode(bd.status, ''H'', ''Y'', ''N'') AS pending_flag --08.10.2018 Inc83
...
SELECT bt.transaction_id  bt_transaction_id,
    ...
    (SELECT to_date(bi.attrib_4,''YYYY-MM-DD'')
     ...
     FETCH FIRST ROW ONLY) AS doc_start_date,
    --insis_blc_global_cust.cust_acc_util_pkg.Get_Doc_End_Date(bd.doc_id) doc_end_date,
    (SELECT to_date(bi.attrib_5,''YYYY-MM-DD'') + 1 -- LPVS-12
     FROM blc_installments bi
     WHERE bi.transaction_id = bt.transaction_id
     FETCH FIRST ROW ONLY) AS doc_end_date,
    insis_blc_global_cust.cust_acc_util_pkg.Get_Ref_Doc_Number(bd.doc_id) ref_doc_number,
    ...
    (SELECT trunc(pp.insr_begin)
     ...
     FETCH FIRST ROW ONLY) AS policy_start_date,
     */
     trunc(pp.insr_begin) AS policy_start_date,
    --insis_blc_global_cust.cust_acc_util_pkg.Get_Policy_End_Date(bd.doc_id) policy_end_date,
    /*
    (SELECT trunc(pp.insr_end) -- LPVS-12 remove - 1
     FROM blc_installments bi,
     ...
     FETCH FIRST ROW ONLY) AS policy_end_date,
    */ 
    trunc(pp.insr_end) AS policy_end_date, 
    insis_blc_global_cust.cust_acc_util_pkg.Get_Pol_End_Flag(pp.policy_id) pol_end_flag,
    blp.lookup_code       pay_way,
    insis_blc_global_cust.cust_acc_util_pkg.Get_Office_Code(nvl(pp.attr4,pp.office_id)) office_gl_no,
    insis_blc_global_cust.cust_acc_util_pkg.Get_Pol_End_Party_Name(pp.policy_id) pol_end_party_name,
