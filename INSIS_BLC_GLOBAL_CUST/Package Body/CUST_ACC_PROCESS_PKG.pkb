CREATE OR REPLACE PACKAGE BODY INSIS_BLC_GLOBAL_CUST.CUST_ACC_PROCESS_PKG AS
...
C_DEFAULT_MODULE      CONSTANT VARCHAR2(240) := 'cust_acc_process_pkg';
--==============================================================================

--------------------------------------------------------------------------------
-- Name: cust_acc_process_pkg.Check_Events_Status
--
-- Type: PROCEDURE
--
...
EXCEPTION WHEN OTHERS THEN
    srv_error.SetSysErrorMsg(l_SrvErrMsg, 'cust_acc_process_pkg.Check_Events_Status', SQLERRM);
...
--------------------------------------------------------------------------------
-- Name: cust_acc_process_pkg.Check_Events_Status_2
--
-- Type: PROCEDURE
--
-- Subtype: DATA_PROCESSING
...
BEGIN
    ...
    --
EXCEPTION WHEN OTHERS THEN
    srv_error.SetSysErrorMsg(l_SrvErrMsg, 'cust_acc_process_pkg.Check_Events_Status_2', SQLERRM);
    srv_error.SetErrorMsg(l_SrvErrMsg, pio_Err);
    blc_log_pkg.insert_message(l_log_module, C_LEVEL_EXCEPTION, SQLERRM);
END Check_Events_Status_2;

--------------------------------------------------------------------------------
-- Name: cust_acc_process_pkg.Set_Events_Status_Z
--
-- Type: PROCEDURE
--
-- Subtype: DATA_PROCESSING
...
IS
    ...
    l_SrvErrMsg     SrvErrMsg;
BEGIN
    -- add two cases to be sure that delete does not lock because of contsraint
    /*
    ...
    */
    ...
    WHERE EXISTS (SELECT 'EVENT'
                  ...
                  WHERE be.attrib_9 = pi_attrib_9
                  AND be.event_id = bg.event_id)
    AND REVERSED_GLTRANS_ID IS NOT NULL;
    ...
    --
EXCEPTION WHEN OTHERS THEN
    srv_error.SetSysErrorMsg(l_SrvErrMsg, 'cust_acc_process_pkg.Set_Events_Status_Z_2', SQLERRM);
    srv_error.SetErrorMsg(l_SrvErrMsg, pio_Err);
    blc_log_pkg.insert_message(l_log_module, C_LEVEL_EXCEPTION, SQLERRM);
END Set_Events_Status_Z_2;

--------------------------------------------------------------------------------
-- Name: cust_acc_process_pkg.Set_Events_Status_Z_2
--
-- Type: PROCEDURE
--
-- Subtype: DATA_PROCESSING
...
BEGIN
    ...
    */
    ...
    DELETE blc_gl_insis2gl
    WHERE event_id IN ( SELECT * FROM TABLE(pi_event_ids) ) AND REVERSED_GLTRANS_ID IS NOT NULL;

    DELETE blc_gl_insis2gl
    WHERE event_id IN ( SELECT * FROM TABLE(pi_event_ids) ) AND REVERSED_GLTRANS_ID IS NULL;
    --
    UPDATE blc_sla_event_status SET event_status = cust_gvar.ACC_MANUALPROCESS
    WHERE event_id IN ( SELECT * FROM TABLE(pi_event_ids) );
    --
EXCEPTION WHEN OTHERS THEN
    srv_error.SetSysErrorMsg(l_SrvErrMsg, 'cust_acc_process_pkg.Set_Events_Status_Z', SQLERRM);
    srv_error.SetErrorMsg(l_SrvErrMsg, pio_Err);
    blc_log_pkg.insert_message(l_log_module, C_LEVEL_EXCEPTION, SQLERRM);
END Set_Events_Status_Z;

--------------------------------------------------------------------------------
-- Name: cust_acc_process_pkg.Create_Proforma_Voucher
--
-- Type: PROCEDURE
--
-- Subtype: DATA_PROCESSING
...
-- Versioning:
--     Fadata   24.10.2017  creation
--     Fadata   08.10.2018  changed - exclude hold documents from voucher creation
--
...
BEGIN
    ...
    FOR c_doc IN (SELECT bt.doc_id,
                         ...
                         be.reversal_flag,
                         be.event_date,
                         --LISTAGG(be.event_id, ',') WITHIN GROUP (ORDER BY be.event_id) event_list, --13.09.2018 remove event list
                         bt.org_id,
                         DECODE(be.reversal_flag,'N','1','2') reversal_order,
                         DECODE(bt.transaction_type,'SIP','SIP','LOANINTRST','LOAN','PROFORMA') bill_type
                  FROM  blc_sla_events be,
                        blc_transactions bt,
                        blc_documents bd
                  ...
                  AND bt.transaction_type IN ('PREMIUM','PREMIUM_SAVING','VAT',
                                                                      'ISSUETAX', --03.07.2019 added for phase 2
                                                                      'SIP','LOANINTRST', --27.11.2019 added for phase 2
                                                                      'MANUAL_PREMIUM_ADJ' ) --16.09.2021 -- LAP85-132
                  AND bt.doc_id = bd.doc_id
                  AND bd.status <> cust_gvar.STATUS_HOLD --08.10.2018 Inc83
                  ...
                  GROUP BY be.legal_entity, be.reversal_flag, be.event_date, bt.doc_id, bt.org_id,
                           DECODE(bt.transaction_type,'SIP','SIP','LOANINTRST','LOAN','PROFORMA')
                  UNION ALL
                  SELECT bt.doc_id,
                         be.legal_entity,
                         be.reversal_flag,
                         be.event_date,
                         --LISTAGG(be.event_id, ',') WITHIN GROUP (ORDER BY be.event_id) event_list, --start 13.09.2018 remove event list
                         bt.org_id,
                         DECODE(be.reversal_flag,'N','1','2') reversal_order,
                         DECODE(bt.transaction_type,'SIP','SIP','LOANINTRST','LOAN','PROFORMA') bill_type
                  FROM  blc_sla_events be,
    ...
    LOOP
       ...
       THEN
          CASE c_doc.bill_type
            WHEN 'LOAN' THEN l_prefix := 'I004';
            WHEN 'SIP' THEN l_prefix := 'I006';
            ELSE l_prefix := 'I001-CRE';
          END CASE;
          l_ref_doc_id := NULL;
       ELSE
          IF c_doc.bill_type IN ('LOAN','SIP')
          THEN
             CONTINUE; --does not create vouchers for deletion of LOAN and SIP bills
          END IF;
         ...
         EXCEPTION
            WHEN OTHERS THEN
              srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Create_Proforma_Voucher', 'cust_acc_process_pkg.CPV.No_Ref_Doc', 'I001-CRE|'||c_doc.doc_id );
              srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
       ...
       --l_event_ids := blc_common_pkg.convert_list(c_doc.event_list);  --start 13.09.2018 remove event list
       ...
       --start 13.09.2018 remove event list
       /*
       ...
       */
       ...
       FOR c_event IN ( SELECT be.event_id
                        ...
                        AND ((be.reversal_flag = cust_gvar.FLG_NO
                         ...
                         AND DECODE(bt.transaction_type,'SIP','SIP','LOANINTRST','LOAN','PROFORMA') = c_doc.bill_type)
       LOOP
          UPDATE blc_sla_events
          SET attrib_9 = po_acc_doc_id
          WHERE event_id = c_event.event_id;
       END LOOP;
       --end 13.09.2018 remove event list
...
EXCEPTION WHEN OTHERS THEN
    srv_error.SetSysErrorMsg(l_SrvErrMsg, 'cust_acc_process_pkg.Create_Proforma_Voucher', SQLERRM);
    srv_error.SetErrorMsg(l_SrvErrMsg, pio_Err);
    blc_log_pkg.insert_message(l_log_module, C_LEVEL_EXCEPTION, 'pi_doc_id = '||pi_doc_id||' - '||SQLERRM);
END Create_Proforma_Voucher;

--------------------------------------------------------------------------------
-- Name: cust_acc_process_pkg.Create_Payment_Voucher
--
-- Type: PROCEDURE
--
-- Subtype: DATA_PROCESSING
...
BEGIN
    ...
    FOR c_doc IN (SELECT bp.payment_id,
                  ...
                  AND bu.attrib_0 IN ('CLAIM','LOAN','SURRENDER','MATURITY','PARTSRNDR') --,'PA??DUPSRNDR')
                  AND ((be.reversal_flag = cust_gvar.FLG_NO
                        AND be.attrib_9 IS NULL
                        AND NOT EXISTS (SELECT 'REV'
                                        FROM blc_sla_events be1
                     ...
                     OR
                       (be.reversal_flag = cust_gvar.FLG_YES
                        ...
                        AND EXISTS ( SELECT 'REV'
                                     ...
                                     AND be1.attrib_9 IS NOT NULL
                                     AND be1.event_id = be.previous_event)))
                  GROUP BY be.legal_entity, be.reversal_flag, be.event_date,
                           bp.payment_id, bp.org_id, bu.attrib_0
                  UNION ALL
                  ...
                  WHERE pi_payment_id IS NOT NULL
                  AND bp.payment_id = pi_payment_id
                  AND be.event_type = 'PAYMENT_CLEARED'
                  AND bp.payment_id = be.payment_id
                  AND bp.usage_id = bu.usage_id
                  AND bu.attrib_0 = 'PAIDUPSRNDR'
                  AND be.reversal_flag = cust_gvar.FLG_NO
                  AND be.attrib_9 IS NULL
                  GROUP BY be.legal_entity, be.event_date,
                           bp.payment_id, bp.org_id,
                           bu.attrib_0
                  UNION ALL
                  SELECT bp.payment_id,
                         be.legal_entity,
    ...
    LOOP
       ...
       IF c_doc.reversal_flag = cust_gvar.FLG_NO
       THEN
          CASE c_doc.pmnt_type
            WHEN 'CLAIM' THEN l_prefix := 'I009';
            WHEN 'LOAN' THEN l_prefix := 'I020';
            WHEN 'SURRENDER' THEN l_prefix := 'I014';
            WHEN 'MATURITY' THEN l_prefix := 'I014';
            WHEN 'PARTSRNDR' THEN l_prefix := 'I014';
            WHEN 'PAIDUPSRNDR' THEN l_prefix := 'I005';
            ELSE NULL;
          END CASE;
       ...
       ELSE
          CASE c_doc.pmnt_type
            WHEN 'CLAIM' THEN l_prefix := 'I011-1'; l_prefix_orig := 'I009';
            WHEN 'LOAN' THEN l_prefix := 'I022'; l_prefix_orig := 'I020';
            WHEN 'SURRENDER' THEN l_prefix := 'I016'; l_prefix_orig := 'I014';
            WHEN 'MATURITY' THEN l_prefix := 'I016'; l_prefix_orig := 'I014';
            WHEN 'PARTSRNDR' THEN l_prefix := 'I016'; l_prefix_orig := 'I014';
            ELSE NULL;
          END CASE;
          ...
          BEGIN
            ...
            AND bd.doc_prefix = l_prefix_orig
            AND bd.doc_type_id = bl.lookup_id
            AND bl.lookup_code = cust_gvar.DOC_ACC_TYPE;
         EXCEPTION
            WHEN OTHERS THEN
              srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Create_Payment_Voucher', 'cust_acc_process_pkg.CPV.No_Ref_Doc', 'I009|'||c_doc.payment_id );
              srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
              blc_log_pkg.insert_message(l_log_module,
                                         C_LEVEL_EXCEPTION,
...
EXCEPTION WHEN OTHERS THEN
    srv_error.SetSysErrorMsg(l_SrvErrMsg, 'cust_acc_process_pkg.Create_Payment_Voucher', SQLERRM);
    srv_error.SetErrorMsg(l_SrvErrMsg, pio_Err);
    blc_log_pkg.insert_message(l_log_module, C_LEVEL_EXCEPTION, 'pi_payment_id = '||pi_payment_id||' - '||SQLERRM);
END Create_Payment_Voucher;

--------------------------------------------------------------------------------
-- Name: cust_acc_process_pkg.Create_Claim_Adj_Voucher
--
-- Type: PROCEDURE
--
-- Subtype: DATA_PROCESSING
...
BEGIN
    ...
    FOR c_doc IN ( SELECT bi.external_id,
                       ...
                       LISTAGG(be.event_id, ',') WITHIN GROUP (ORDER BY be.event_id) event_list,
                       ba.party --26.10.2018 --add to split vouchers for insurers
                   FROM blc_sla_events be,
                        blc_installments bi,
                        blc_items i,
                        blc_accounts ba
                   WHERE be.legal_entity = pi_le_id
                       ...
                       AND bi.account_id = ba.account_id --26.10.2018 --add to split vouchers for insurers
    ...
    LOOP
        ...
        blc_spec_doc_util_pkg.Create_Document
               ( pi_legal_entity_id     =>   c_doc.legal_entity,
                 ...
                 pi_attrib_0            =>   NULL,
                 pi_attrib_1            =>   NULL,
                 pi_attrib_2            =>   c_doc.party, --26.10.2018 --add to split vouchers for insurers
                 pi_attrib_3            =>   NULL,
                 pi_attrib_4            =>   NULL,
                 pi_attrib_5            =>   NULL,
                 pi_attrib_6            =>   NULL,
    ...
    --
EXCEPTION WHEN OTHERS THEN
    srv_error.SetSysErrorMsg(l_SrvErrMsg, 'cust_acc_process_pkg.Create_Claim_Adj_Voucher', SQLERRM);
    srv_error.SetErrorMsg(l_SrvErrMsg, pio_Err);
    blc_log_pkg.insert_message(l_log_module, C_LEVEL_EXCEPTION, 'pi_le_id = '||pi_le_id||' - '||SQLERRM);
END Create_Claim_Adj_Voucher;

--------------------------------------------------------------------------------
-- Name: cust_acc_process_pkg.Get_Org_Id
--
-- Type: FUNCTION
--
-- Subtype: DATA_GET
...
--------------------------------------------------------------------------------
-- Name: cust_acc_process_pkg.Create_Reserve_Voucher
--
-- Type: PROCEDURE
--
-- Subtype: DATA_PROCESSING
...
IS
    ...
    l_org_id        NUMBER;
    l_prefix        VARCHAR2(30);
    --l_event_ids     BLC_SELECTED_OBJECTS_TABLE; --commented by Maria on 06.07.2018 - LPV-1674
    l_acc_doc_id    NUMBER;
    l_doc_number    VARCHAR2(100);
    --
    l_doc           BLC_DOCUMENTS_TYPE;
BEGIN
    ...
    FOR c_doc IN ( SELECT be.legal_entity,
                       ...
                       DECODE( be.event_type, 'EXTAVINCR', '01',
                                              ...
                                              'EXTIBNRCR', '08',
                                              'EXTCEXPALL', '09',
                                              'EXTCEXPUNALL', '10' ) AS event_type
                       --LISTAGG(be.event_id, ',') WITHIN GROUP (ORDER BY be.event_id) event_list --commented by Maria on 06.07.2018 - LPV-1674
                   FROM blc_sla_events be,
    ...
    LOOP
        ...
        --
        --l_event_ids := blc_common_pkg.convert_list(c_doc.event_list); --commented by Maria on 06.07.2018 - LPV-1674
        ...
        blc_spec_doc_util_pkg.Create_Document
               ( pi_legal_entity_id     =>   c_doc.legal_entity,
                 ...
                 pi_doc_prefix          =>   l_prefix,
                 pi_doc_number          =>   NULL,
                 pi_doc_suffix          =>   c_doc.event_type, --26.10.2018
                 pi_reference           =>   NULL,
                 pi_ref_doc_id          =>   NULL,
                 pi_run_id              =>   NULL,
                 pi_direction           =>   NULL,
                 pi_attrib_0            =>   NULL,
                 pi_attrib_1            =>   NULL,
                 pi_attrib_2            =>   c_doc.office, --26.10.2018
                 pi_attrib_3            =>   c_doc.currency, --26.10.2018
                 pi_attrib_4            =>   NULL,
                 pi_attrib_5            =>   NULL,
                 pi_attrib_6            =>   NULL,
                 pi_attrib_7            =>   NULL,
        ...
        THEN
            ...
            RETURN;
        END IF;
        --added by Maria on 06.07.2018 - LPV-1674
        FOR c_event IN (  SELECT be.event_id
                          FROM blc_sla_events be,
                              blc_staging_area bsa
                          WHERE be.legal_entity = pi_le_id
                             ...
                             AND be.legal_entity = c_doc.legal_entity
                             AND be.event_date = c_doc.event_date
                             AND bsa.c_attrib_2 = c_doc.currency
                             AND bsa.n_attrib_4 = c_doc.rate
                             AND ( c_doc.office IS NULL OR bsa.c_attrib_11 = c_doc.office )
                             AND DECODE( be.event_type, 'EXTAVINCR', '01',
                                                    'EXTGRINCR', '02', 'EXTNRINCR', '02', 'EXTZRINCR', '02',
                                                    'EXTUPRINCR', '03', --'EXTDACINCR', '03',
                                                    'EXTRIUPRINCR', '05', --'EXTRIDACINCR', '05',
                                                    'EXTIBNRGR', '07',
                                                    'EXTIBNRCR', '08',
                                                    'EXTCEXPALL', '09',
                                                    'EXTCEXPUNALL', '10' ) = c_doc.event_type )
       LOOP
          UPDATE blc_sla_events
          SET attrib_9 = l_acc_doc_id
          WHERE event_id = c_event.event_id;
       ...
       --commented by Maria on 06.07.2018 - LPV-1674
    ...
    --
EXCEPTION WHEN OTHERS THEN
    srv_error.SetSysErrorMsg(l_SrvErrMsg, 'cust_acc_process_pkg.Create_Reserve_Voucher', SQLERRM);
    srv_error.SetErrorMsg(l_SrvErrMsg, pio_Err);
    blc_log_pkg.insert_message(l_log_module, C_LEVEL_EXCEPTION, 'pi_le_id = '||pi_le_id||' - '||SQLERRM);
END Create_Reserve_Voucher;

--------------------------------------------------------------------------------
-- Name: cust_acc_process_pkg.Create_Claim_AC_Voucher
--
-- Type: PROCEDURE
--
-- Subtype: DATA_PROCESSING
...
BEGIN
    ...
    --
EXCEPTION WHEN OTHERS THEN
    srv_error.SetSysErrorMsg(l_SrvErrMsg, 'cust_acc_process_pkg.Create_Claim_AC_Voucher', SQLERRM);
    srv_error.SetErrorMsg(l_SrvErrMsg, pio_Err);
    blc_log_pkg.insert_message(l_log_module, C_LEVEL_EXCEPTION, 'pi_le_id = '||pi_le_id||' - '||SQLERRM);
END Create_Claim_AC_Voucher;

--------------------------------------------------------------------------------
-- Name: cust_acc_process_pkg.Create_Prem_AC_Voucher
--
-- Type: PROCEDURE
--
-- Subtype: DATA_PROCESSING
...
BEGIN
    ...
    FOR c_doc IN ( SELECT i.component policy_id,
                   ...
                   WHERE be.legal_entity = pi_le_id
                       ...
                       AND bi.installment_type IN ('BCPRCOA','BCLFCOA','BCCOMMCOA',
                                                   'BCLFAOCOA','BCLFCSCOA','BCLFCFCOA','BCLFDCCOA') --03.07.2019 added for phase 2
    ...
    --
EXCEPTION WHEN OTHERS THEN
    srv_error.SetSysErrorMsg(l_SrvErrMsg, 'cust_acc_process_pkg.Create_Prem_AC_Voucher', SQLERRM);
    srv_error.SetErrorMsg(l_SrvErrMsg, pio_Err);
    blc_log_pkg.insert_message(l_log_module, C_LEVEL_EXCEPTION, 'pi_le_id = '||pi_le_id||' - '||SQLERRM);
END Create_Prem_AC_Voucher;

--------------------------------------------------------------------------------
-- Name: cust_acc_process_pkg.Create_Claim_CORI_Voucher
--
-- Type: PROCEDURE
--
-- Subtype: DATA_PROCESSING
...
BEGIN
    ...
    LOOP
       ...
       ELSE
         ...
         EXCEPTION
            WHEN OTHERS THEN
              srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Create_Claim_CORI_Voucher', 'cust_acc_process_pkg.CPV.No_Ref_Doc', l_prefix_right||'|'||c_doc.clearing_id||' / '||c_doc.party );
              srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
              blc_log_pkg.insert_message(l_log_module,
                                         C_LEVEL_EXCEPTION,
...
EXCEPTION WHEN OTHERS THEN
    srv_error.SetSysErrorMsg(l_SrvErrMsg, 'cust_acc_process_pkg.Create_Claim_CORI_Voucher', SQLERRM);
    srv_error.SetErrorMsg(l_SrvErrMsg, pio_Err);
...
--------------------------------------------------------------------------------
-- Name: cust_acc_process_pkg.Create_RI_Bill_Voucher
--
-- Type: PROCEDURE
--
-- Subtype: DATA_PROCESSING
...
--     Fadata   29.08.2018  changed - add case from 'RI_EXPENSES' - LPV-1791
--
-- Purpose: Create a document for the accounting events for given document
--
-- Input parameters:
...
BEGIN
    ...
    FOR c_doc IN (SELECT bt.doc_id,
                  ...
                  AND be.attrib_9 IS NULL
                  AND bt.transaction_id = be.transaction_id
                  AND bt.transaction_type IN ('RI_PREMIUM','RI_COMMISSION','RI_TAX','RI_EXPENSES') -- LPV-1791
    ...
    LOOP
       ...
       ELSE
         ...
         EXCEPTION
            WHEN OTHERS THEN
              srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Create_Claim_CORI_Voucher', 'cust_acc_process_pkg.CPV.No_Ref_Doc', 'I018'||'|'||c_doc.doc_id||' / '||c_doc.office );
              srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
              blc_log_pkg.insert_message(l_log_module,
                                         C_LEVEL_EXCEPTION,
...
EXCEPTION WHEN OTHERS THEN
    srv_error.SetSysErrorMsg(l_SrvErrMsg, 'cust_acc_process_pkg.Create_RI_Bill_Voucher', SQLERRM);
    srv_error.SetErrorMsg(l_SrvErrMsg, pio_Err);
    blc_log_pkg.insert_message(l_log_module, C_LEVEL_EXCEPTION, 'pi_doc_id = '||pi_doc_id||' - '||SQLERRM);
END Create_RI_Bill_Voucher;

--------------------------------------------------------------------------------
-- Name: cust_acc_process_pkg.Validate_Doc_Acc_Trx
--
-- Type: PROCEDURE
--
-- Subtype: DATA_PROCESSING
...
BEGIN
    ...
    THEN
       srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Validate_Doc_Acc_Trx', 'cust_acc_process_pkg.VDAT.WrongStatus', pi_doc_id||'|'||l_doc.status);
       srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
    ...
    THEN
       srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Validate_Doc_Acc_Trx', 'cust_acc_process_pkg.VDAT.Miss_CO_Inst');
       srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
       blc_log_pkg.insert_message(l_log_module, C_LEVEL_EXCEPTION,
                                  'pi_doc_id = '||pi_doc_id||' - '||
    ...
    THEN
       srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Validate_Doc_Acc_Trx', 'cust_acc_process_pkg.VDAT.Diff_BLC001_amount', l_BLC001_amount_trx||'/'||l_BLC001_amount_acc );
       srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
       blc_log_pkg.insert_message(l_log_module, C_LEVEL_EXCEPTION,
                                  'pi_doc_id = '||pi_doc_id||' - '||
    ...
    THEN
       srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Validate_Doc_Acc_Trx', 'cust_acc_process_pkg.VDAT.Diff_BLC002_amount', l_BLC002_amount_trx||'/'||l_BLC002_amount_acc );
       srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
       blc_log_pkg.insert_message(l_log_module, C_LEVEL_EXCEPTION,
                                  'pi_doc_id = '||pi_doc_id||' - '||
    ...
    THEN
       srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Validate_Doc_Acc_Trx', 'cust_acc_process_pkg.VDAT.Diff_BLC003_amount', l_BLC003_amount_trx||'/'||l_BLC003_amount_acc );
       srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
       blc_log_pkg.insert_message(l_log_module, C_LEVEL_EXCEPTION,
                                  'pi_doc_id = '||pi_doc_id||' - '||
    ...
    THEN
       srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Validate_Doc_Acc_Trx', 'cust_acc_process_pkg.VDAT.Diff_BLC005_amount', l_BLC005_amount_trx||'/'||l_BLC005_amount_acc );
       srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
       blc_log_pkg.insert_message(l_log_module, C_LEVEL_EXCEPTION,
                                  'pi_doc_id = '||pi_doc_id||' - '||
...
--------------------------------------------------------------------------------
-- Name: cust_acc_process_pkg.Validate_Pmnt_Acc_Trx
--
-- Type: PROCEDURE
--
-- Subtype: DATA_PROCESSING
...
BEGIN
    ...
    THEN
       srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Validate_Pmnt_Acc_Trx', 'cust_acc_process_pkg.VPAT.WrongStatus', pi_payment_id||'|'||l_payment.status);
       srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
       blc_log_pkg.insert_message(l_log_module, C_LEVEL_EXCEPTION,
...
--------------------------------------------------------------------------------
-- Name: cust_acc_process_pkg.Process_Acc_Trx
--
-- Type: PROCEDURE
--
-- Subtype: DATA_PROCESSING
...
-- Versioning:
--     Fadata   24.10.2017  creation
--     Fadata   19.04.2019  changed - add check if exist locked data or period
--                          LPVS-118
--     Fadata   16.05.2019  changed - add check to not create vouchers when
--                          exist unprocessed accounting events
--                          LPVS-118
--     Fadata   15.06.2019  changed - move check for unprocessed accounting
--                          events after create accounting for immediate call
--                          LPVS-118
--
...
PROCEDURE Process_Acc_Trx( pi_acc_doc_id   IN     NUMBER,
                           ...
                           pi_batch_flag   IN     VARCHAR2 DEFAULT cust_gvar.FLG_NO,
                           pio_Err         IN OUT SrvErr )
IS
    l_log_module      VARCHAR2(240);
    l_SrvErrMsg       SrvErrMsg;
    ...
    l_count_unproc     PLS_INTEGER; --LPVS-118
BEGIN
    ...
    --begin LPVS-118
    IF pi_batch_flag = cust_gvar.FLG_YES
    THEN
       IF Is_Date_Period_Lock = cust_gvar.FLG_YES
       THEN
          blc_log_pkg.insert_message(l_log_module, C_LEVEL_STATEMENT,
                                    'There is an active close date or month procedure');
          RETURN;
       END IF;
    END IF;
    --end LPVS-118
    ...
    */
    ...
    LOOP
       ...
       IF l_count_e = 0
       THEN
          srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Process_Acc_Trx', 'cust_acc_process_pkg.CCDAT.No_Acc_Events' );
          srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
          blc_log_pkg.insert_message(l_log_module, C_LEVEL_EXCEPTION,
       ...
       THEN
          ...
          THEN
             srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Process_Acc_Trx', 'cust_acc_process_pkg.PAT.ReversedNotProcess', c_doc.ref_doc_id);
             srv_error.SetErrorMsg( l_SrvErrMsg, l_SrvErr );
             l_err_flag := cust_gvar.FLG_YES;
          ELSIF l_doc_ref.status = cust_gvar.STATUS_REJECT
          THEN
             srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Process_Acc_Trx', 'cust_acc_process_pkg.PAT.ReversedReject', c_doc.ref_doc_id);
             srv_error.SetErrorMsg( l_SrvErrMsg, l_SrvErr );
             l_err_flag := cust_gvar.FLG_YES;
          END IF;
       END IF;
       ...
       INTO l_count_unproc
       FROM blc_sla_events be,
            blc_sla_event_status bes
       WHERE be.attrib_9 = to_char(c_doc.acc_doc_id)
       AND be.event_id = bes.event_id
       AND bes.event_status = 'U';
       ...
       END IF;
       --
       IF c_doc.doc_prefix IN ( 'I001-CRE', 'I006' ) -- add SIP event (I006) 24.01.2020 phase 2 LPV-2163
       THEN
          /* --16.09.2018 - no need to do validate, works very slowly
          IF l_err_flag = cust_gvar.FLG_NO
          THEN
          ...
          */
          ...
          THEN
             blc_doc_process_pkg.Set_Formal_Document
                  (pi_doc_id             => c_doc.doc_suffix,
                   pi_action_notes       => srv_error.GetSrvMessage('cust_acc_process_pkg.PAT.IntInsert')||' '||l_header_id,
       ...
       THEN
          ...
          cust_acc_export_pkg.Insert_SAP_Delete_Proforma
           ...
           THEN
              l_err_flag := cust_gvar.FLG_YES;
           END IF;
       ELSIF c_doc.doc_prefix IN ('I009','I014','I020','I005')
       THEN
          l_break := '008';
          IF l_err_flag = cust_gvar.FLG_NO AND c_doc.doc_prefix IN ('I009','I014','I020')
          THEN
          ...
          IF l_err_flag = cust_gvar.FLG_NO
          THEN
             IF c_doc.doc_prefix = 'I020'
             THEN
                cust_acc_export_2_pkg.Insert_SAP_Loan_Pmnt
                   (pi_payment_id     => c_doc.doc_suffix,
                    pi_acc_doc_id     => c_doc.acc_doc_id,
                    po_header_id      => l_header_id,
                    pio_Err           => l_SrvErr);
          ...
          THEN
             blc_pmnt_util_pkg.Change_Payments_Status
                 (pi_payment_ids    => c_doc.doc_suffix,
                  ...
                  pi_notes          => srv_error.GetSrvMessage('cust_acc_process_pkg.PAT.IntInsert')||' '||l_header_id,
             ...
             END IF;
          END IF;
       ELSIF c_doc.doc_prefix IN ('I011-1','I016','I022')
       THEN
          l_break := '010';
          IF l_err_flag = cust_gvar.FLG_NO
          THEN
             IF c_doc.doc_prefix = 'I022'
             THEN
                cust_acc_export_2_pkg.Insert_SAP_Rvrs_Loan_Pmnt
                   (pi_payment_id     => c_doc.doc_suffix,
                    pi_acc_doc_id     => c_doc.acc_doc_id,
                    pi_reversed_id    => l_doc_ref.doc_number,
                    po_header_id      => l_header_id,
       ...
       THEN
          ...
          cust_acc_export_pkg.Insert_SAP_Delete_RI_Bill
           ...
           THEN
              l_err_flag := cust_gvar.FLG_YES;
           END IF;
       ELSIF c_doc.doc_prefix IN ('I017','I019')
       THEN
          l_break := '019';
          IF l_err_flag = cust_gvar.FLG_NO
          THEN
             cust_acc_export_2_pkg.Insert_SAP_CORI_Pmnt
                   (pi_payment_id     => c_doc.doc_suffix,
                    pi_acc_doc_id     => c_doc.acc_doc_id,
                    po_header_id      => l_header_id,
                    pio_Err           => l_SrvErr);
             ...
             END IF;
          END IF;
       ELSIF c_doc.doc_prefix = 'I004'
       THEN
          l_break := '020';
          IF l_err_flag = cust_gvar.FLG_NO
          THEN
             cust_acc_export_2_pkg.Insert_SAP_Loan_Bill
                   (pi_doc_id         => c_doc.doc_suffix,
                    pi_acc_doc_id     => c_doc.acc_doc_id,
                    po_header_id      => l_header_id,
                    pio_Err           => l_SrvErr);
       ...
       ELSE
          ...
          THEN
             l_action_notes := srv_error.GetSrvMessage('cust_acc_process_pkg.PAT.OldNumber')||' '||l_doc.doc_number||'; '||srv_error.GetSrvMessage('cust_acc_process_pkg.PAT.IntInsert')||' '||l_header_id;
          ELSE
             l_action_notes := srv_error.GetSrvMessage('cust_acc_process_pkg.PAT.IntInsert')||' '||l_header_id;
          END IF;
    ...
    --
EXCEPTION WHEN OTHERS THEN
    srv_error.SetSysErrorMsg(l_SrvErrMsg, 'cust_acc_process_pkg.Process_Acc_Trx', l_break||' - '||SQLERRM);
    srv_error.SetErrorMsg(l_SrvErrMsg, pio_Err);
    blc_log_pkg.insert_message(l_log_module, C_LEVEL_EXCEPTION, 'pi_acc_doc_id = '||pi_acc_doc_id||' - '|| l_break||' - '||SQLERRM);
END Process_Acc_Trx;

--------------------------------------------------------------------------------
-- Name: cust_acc_process_pkg.Set_Result_Acc_Trx
--
-- Type: PROCEDURE
--
-- Subtype: DATA_PROCESSING
...
BEGIN
    ...
    THEN
       ...
       IF l_count_e = 0
       THEN
          srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Set_Result_Acc_Trx', 'cust_acc_process_pkg.CCDAT.No_Acc_Events' );
          srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
          RETURN;
       END IF;
       ...
       THEN
          ...
          EXCEPTION
             WHEN OTHERS THEN
               srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Set_Result_Acc_Trx', 'blc_doc_util_pkg.VDO.Inv_DocId', l_doc.doc_suffix);
               srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
               ROLLBACK TO DOC_UPDATE;
               RETURN;
          END;
          ...
          THEN
           ...
           THEN
              ...
              THEN
                 srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Set_Result_Acc_Trx', 'cust_acc_process_pkg.SRAT.Exist_Rev', l_doc_orig.status);
                 srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
                 ROLLBACK TO DOC_UPDATE;
                 RETURN;
              END IF;
           ELSE
              srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Set_Result_Acc_Trx', 'cust_acc_process_pkg.SRAT.Inv_DocStatus', l_rev_header);
              srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
              ROLLBACK TO DOC_UPDATE;
              RETURN;
          END IF;
       ELSIF l_doc.doc_prefix IN ('I009','I014','I020')
       THEN
          ...
          THEN
             srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Set_Result_Acc_Trx', 'cust_acc_process_pkg.SRAT.Inv_PmntStatus', l_pmnt_orig.status);
             srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
             ROLLBACK TO DOC_UPDATE;
             RETURN;
          END IF;
          ...
          THEN
             ...
             RETURN;
          END IF;
       ELSIF l_doc.doc_prefix IN ('I004','I006')
       THEN
          BEGIN
             l_doc_orig := blc_documents_type(l_doc.doc_suffix);
          EXCEPTION
    ...
    THEN
       ...
       THEN
          ...
          THEN
             ...
             RETURN;
          END IF;
       ELSIF l_doc.doc_prefix IN ('I009','I014','I020','I005')
       THEN
          l_pmnt_orig := blc_payments_type(l_doc.doc_suffix, pio_Err);
          IF NOT srv_error.rqStatus( pio_Err )
          THEN
       ...
       THEN
          ...
          THEN
             ...
             EXCEPTION
                WHEN OTHERS THEN
                  srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Set_Result_Acc_Trx', 'blc_doc_util_pkg.VDO.Inv_DocId', l_doc_ref.doc_suffix);
                  srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
                  ROLLBACK TO DOC_UPDATE;
                  RETURN;
             END;
             ...
             THEN
                ...
                RETURN;
             END IF;
          ELSIF l_doc_ref.doc_prefix IN ('I009','I014','I020','I005')
          THEN
    ...
    --
EXCEPTION WHEN OTHERS THEN
    srv_error.SetSysErrorMsg(l_SrvErrMsg, 'cust_acc_process_pkg.Set_Result_Acc_Trx', SQLERRM);
    srv_error.SetErrorMsg(l_SrvErrMsg, pio_Err);
    blc_log_pkg.insert_message(l_log_module, C_LEVEL_EXCEPTION, 'pi_acc_doc_id = '||pi_acc_doc_id||' - '|| SQLERRM);
END Set_Result_Acc_Trx;

--------------------------------------------------------------------------------
-- Name: cust_acc_process_pkg.Recreate_Acc_Trx
--
-- Type: PROCEDURE
--
-- Subtype: DATA_PROCESSING
...
BEGIN
    ...
    THEN
       srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Recreate_Acc_Trx', 'blc_doc_util_pkg.VDO.No_DocId' );
       srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
       blc_log_pkg.insert_message(l_log_module,
                                  C_LEVEL_EXCEPTION,
    ...
    EXCEPTION
       WHEN OTHERS THEN
         srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Recreate_Acc_Trx', 'blc_doc_util_pkg.VDO.Inv_DocId', l_doc.doc_suffix);
         srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
         blc_log_pkg.insert_message(l_log_module,
                                    C_LEVEL_EXCEPTION,
                                   'pi_acc_doc_id = '||pi_acc_doc_id||' - '||pio_Err(pio_Err.FIRST).errmessage);
    ...
    THEN
       srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Recreate_Acc_Trx', 'cust_acc_process_pkg.RAT.Inv_DocStatus' );
       srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
       blc_log_pkg.insert_message(l_log_module,
                                  C_LEVEL_EXCEPTION,
                                  'pi_acc_doc_id = '||pi_acc_doc_id||' - '||pio_Err(pio_Err.FIRST).errmessage);
    ...
    THEN
       srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Recreate_Acc_Trx', 'cust_acc_process_pkg.RAT.Inv_DocType' );
       srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
       blc_log_pkg.insert_message(l_log_module,
                                  C_LEVEL_EXCEPTION,
                                  'pi_acc_doc_id = '||pi_acc_doc_id||' - '||pio_Err(pio_Err.FIRST).errmessage);
    ...
    THEN
       ...
       srv_error.SetErrorMsg( l_SrvErrMsg, 'cust_acc_process_pkg.Recreate_Acc_Trx', 'cust_acc_process_pkg.RAT.Accounting_Error', l_action_notes);
       srv_error.SetErrorMsg( l_SrvErrMsg, pio_Err );
    ...
    --
EXCEPTION WHEN OTHERS THEN
    srv_error.SetSysErrorMsg(l_SrvErrMsg, 'cust_acc_process_pkg.Recreate_Acc_Trx', SQLERRM);
    srv_error.SetErrorMsg(l_SrvErrMsg, pio_Err);
    blc_log_pkg.insert_message(l_log_module, C_LEVEL_EXCEPTION, 'pi_acc_doc_id = '||pi_acc_doc_id||' - '||SQLERRM);
END Recreate_Acc_Trx;

--------------------------------------------------------------------------------
-- Name: cust_acc_process_pkg.Is_Date_Period_Lock
--
-- Type: FUNCTION
--
-- Subtype: DATA_GET
...
--
-- Versioning:
--     Fadata   19.04.2019  creation - LPVS-118
--
-- Purpose: Check if there is lock date or period from running procedure for
-- date or month close
--
-- Returns: Y/N
--
-- Usage: Before procedure for processing data to integration tables to avoid
-- getting of transactions with not yet created accounting
--------------------------------------------------------------------------------
FUNCTION Is_Date_Period_Lock
RETURN VARCHAR2
IS
   l_lock_period  NUMBER;
   l_lock_date    DATE;
   l_return_value VARCHAR2(1) DEFAULT cust_gvar.FLG_NO;
BEGIN
   l_lock_date := sys_days.get_lock_date;

   IF l_lock_date IS NOT NULL
   THEN
      l_return_value := cust_gvar.FLG_YES;
   ELSE
      l_lock_period := sys_periods.get_lock_sys_period;
      IF l_lock_period IS NOT NULL
      THEN
         l_return_value := cust_gvar.FLG_YES;
      END IF;
   END IF;
...
END Is_Date_Period_Lock;
--
END CUST_ACC_PROCESS_PKG;
/